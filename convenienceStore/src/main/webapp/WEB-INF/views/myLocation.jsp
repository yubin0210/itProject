		...
		<div class="option">
			<div>
				<form onsubmit="searchPlaces(); return false;">
					키워드 : <input type="text" id="keyword" placeholder="장소 이름을 입력..." size="15">
...
<c:if test="${dto == null}">
	...
	<form action="" id="myLocation" name="myLocation" method="POST">
		...
		<input type="submit" value="선택 후 결정">
	</form>
</c:if>
...
<script>
	...
	var markerClick = new kakao.maps.Marker(),
		infowindowClick = new kakao.maps.InfoWindow({zindex : 1});
	...
	var thisMarker = new kakao.maps.Marker({
		...
		thisInfowindow = new kakao.maps.InfoWindow({zindex : 1});
	...
	function displayPlaces(places) {
		...
		for(var i = 0; i < places.length; i++) {
			...
			itemEl = getListItem(i, places[i]);	// 검색 결과 항목 Element를 생성 합니다
...
// 				};
	...
	function addMarker(position, idx, title) {
		var imageSrc = 'https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/marker_number_blue.png',	// 마커 이미지 url, 스프라이트 이미지를 씁니다
			...
			imgOptions = {
				...
				offset : new kakao.maps.Point(13, 37)	// 마커 좌표에 일치 시킬 이미지 내 에서의 좌표
			},
	...
	function displayInfowindow(marker, title) {
		var content = '<div style="padding: 5px; z-index:1; margin: auto;">' + title + '</div>';
	...
	kakao.maps.event.addListener(map, 'click', function(mouseEvent) { 
		...
		const addInHTML = document.getElementById('add');
	    ...
	    searchDetailAddrFromCoords(mouseEvent.latLng, function(result, status) {
	    	if(status == kakao.maps.services.Status.OK) {
	    		...
	    		// 마커를 클릭한 위치에 표시합니다 
	    	    markerClick.setPosition(mouseEvent.latLng);		
...
// 	    	    infowindowClick.open(map, markerClick);
	    	  	...
	    	  	// mouseover, out 시 인포 윈도우를 출력
	    	   	(function(markerClick, contentClick) {
	    			kakao.maps.event.addListener(markerClick, 'mouseover', function() {
	    				infowindowClick.setContent(contentClick);
	    				infowindowClick.open(map, markerClick);
	    			});
	    					
	    			kakao.maps.event.addListener(markerClick, 'mouseout', function() {
	    	    ...
	    	    document.getElementById('address').innerHTML = "<input type='hidden' name='address' value='" + detailResult + "'>";
