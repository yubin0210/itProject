...
.headerMenu>div>a, div.locationSelect>a {
	font-size: 17px;
}
...
.headerMenu>div, .locationSelect>a {
	margin: 0 10px;
	padding: 5px 0;
	transition: all 0.2s ease-in-out;
}

.headerMenu>div:hover, .locationSelect>a:hover {
	color: #0056b3;
}
			...
			<div class="headerMenu flex">
				...
				<c:if test="${not empty login }">
					...
					<c:if test="${login.role == 1 }">
						<div>
							<a href="${cpath }/member/storeMemberList">점주가입 목록</a>
						</div>
						<div>
							<a href="${cpath }/admin/chat">문의채팅</a>
						</div>
					</c:if>
					<c:if test="${login.role == 3 }">
						<div class="header-cart">
							<a href="${cpath }/payment/cart">장바구니</a>
							<div class="${count == '0' ? '':'header-cart-count' }">${count == '0' ? '':count }</div>
						</div>
						<div>
							<a href="${cpath }/myPage/myPageMain?userid=${login.userid}">마이페이지</a>
						</div>
					</c:if>
					<div>
						<a href="${cpath }/inquiry/list">고객센터</a>
					</div>
				</c:if>
			</div>
		</div>
	...
	<div class="topMenuBorder">
		<div class="topList frame">
			<ul class="flex menuText">
				<li><a href="${cpath }/"><img id="logo"
						src="${cpath }/resources/image/home/로고.png"></a></li>
				...
				<li><a href="${cpath }/project">프로젝트 소개</a></li>
			</ul>
		</div>
	</div>